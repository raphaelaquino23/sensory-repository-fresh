...
export class CommentRepository {
  ...
  constructor() {
    ...
    this.commentInformationRepository = this.db.sequelize.getRepository(CommentInformation);
  ...
  //GET BY ID COMMENT
	...
	async getCommentByPostId(PostId: number) {
		try {
			const comments = await this.commentRepository.findAll({where: {Post_Id: PostId}});
  ...
  async createComment(comment: Comment, commentInformation: CommentInformation, commentStats: CommentStats) {
    let comInfo, comStats, data = {};
    try {
      comment.Comment_DateCreated = new Date();
			comInfo = await this.commentInformationRepository.create(commentInformation);
			comment.CommentInformation_Id = comInfo.getDataValue('CommentInformation_Id');
  ...
  async updateComment(comment: Comment, commentInformation: CommentInformation, commentStats: CommentStats) {
    ...
    try {
      ...
      await this.commentInformationRepository.update({...commentInformation}, {
        where: {
	 				CommentInformation_Id: commentInformation.CommentInformation_Id
        }
      });

      await this.commentStatsRepository.update({...commentStats}, {
        where: {
	 				CommentStats_Id: commentStats.CommentStats_Id
        }
      });

      data = await this.commentRepository.update({...Comment}, {
        where: {
  ...
  async deleteComment(Comment_Id: number){
    let  comRef, comInfo, comStats, data = {};
    try {
			comRef = await this.commentRepository.findOne({where: {Comment_Id: Comment_Id}});
			comInfo = await this.commentInformationRepository.findOne({where: {CommentInformation_Id: comRef.getDataValue("CommentInformation_Id")}});
			comStats = await this.commentStatsRepository.findOne({where: {CommentStats_Id: comRef.getDataValue("CommentStats_Id")}});
  ...
  async getCommentInformation() {
    try {
      const commentInformation = await this.commentInformationRepository.findAll();
	...
	async getCommentInformationById(CommentInformation_Id: number) {
		try {
			const comment = await this.commentInformationRepository.findOne({where: {CommentInformation_Id: CommentInformation_Id}});
			console.log('comments:::', comment);
			return comment;
		} catch (error) {
			console.log("error");
  ...
  async updateCommentInformation(commentInformation: CommentInformation) {
    let data = {};
    try {
      data = await this.commentInformationRepository.update({...CommentInformation}, {
        where: {
          CommentInformation_Id: commentInformation.CommentInformation_Id
        }
      });
  ...
  async updateCommentStats(commentStats: CommentStats) {
    let data = {};
    try {
      data = await this.commentStatsRepository.update({...CommentStats}, {
        where: {
          CommentStats_Id: commentStats.CommentStats_Id
        }
      });
