...
export class MessageRepository {
  ...
  async createMessage(Message: Message, SenderName: String, ReceiverName: String) {
    ...
    try {
      ...
      if(userSearch){
        ...
        console.log("---------------USER ID");
        userId = await this.userRepository.findOne({where: {UserInformation_Id: userSearch.getDataValue("UserInformation_Id")}});
        ...
        if(userId){
          ...
          receiverSearch = await this.userInformationRepository.findOne({where: {UserInformation_Name: ReceiverName}})
          if(receiverSearch){
            ...
            receiverId = await this.userRepository.findOne({where: {UserInformation_Id: receiverSearch.getDataValue("UserInformation_Id")}})
            if(receiverId){
  ...
  async updateMessage(Message: Message) {
    let data = {};
    try {
      data = await this.messageRepository.update({...Message}, {
        where: {
