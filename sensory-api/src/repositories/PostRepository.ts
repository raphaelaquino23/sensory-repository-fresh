...
export class PostRepository {
  ...
  constructor() {
    ...
    this.postInformationRepository = this.db.sequelize.getRepository(PostInformation);    
    ...
    this.postUpvoteTrackerRepository = this.db.sequelize.getRepository(PostUpvoteTracker)
  }
  ...
  async getPostById(Post_Id: number) {
    try {
      const posts = await this.postRepository.findOne({ where: { Post_Id: Post_Id}});
      console.log('posts::: ', posts);
      return posts;
    } catch (error) {
      console.log("error");
...
//          error
  ...
  async updatePost(post: Post, postInformation: PostInformation, postStats: PostStats) {
    ...
    try {
      post.Post_DateEdited = new Date();
			await this.postInformationRepository.update({...postInformation}, {
				where: {
					PostInformation_Id: postInformation.PostInformation_Id
				}
			});      
			await this.postStatsRepository.update({...postStats}, {
				where: {
					PostStats_Id: postStats.PostStats_Id
				}
			});
      data = await this.postRepository.update({...post}, {
        where: {
  ...
  async deletePost(Post_Id: number){
    let pRef, pInfo, pStats, data = {};
    try {
			pRef = await this.postRepository.findOne({where: {Post_Id: Post_Id}});
			pInfo = await this.postInformationRepository.findOne({where: {PostInformation_Id: pRef.getDataValue("PostInformation_Id")}});
			pStats = await this.postStatsRepository.findOne({where: {PostStats_Id: pRef.getDataValue("PostStats_Id")}});
  ...
  async getPostInformationById(PostInformation_Id: number) {
    try {
      const posts = await this.postInformationRepository.findOne({ where: { PostInformation_Id: PostInformation_Id}});
      console.log('posts::: ', posts);
      return posts;
    } catch (error) {
      console.log("error");
  ...
  async updatePostInformation(postinformation: PostInformation) {
    let data = {};
    try {
      data = await this.postInformationRepository.update({...postinformation}, {
        where: {
          PostInformation_Id: postinformation.PostInformation_Id
        }
      });
  ...
  async updatePostCategory(postCategory: PostCategory) {
    let data = {};
    try {
      data = await this.postCategoryRepository.update({...postCategory}, {
        where: {
  ...
  async getPostStatsById(PostStats_Id: number) {
    try {
      const postStats = await this.postStatsRepository.findOne({ where: {PostStats_Id: PostStats_Id }});
  ...
  async updatePostStats(postStats: PostStats) {
    let data = {};
    try {
      data = await this.postStatsRepository.update({...postStats}, {
        where: {
          PostStats_Id: postStats.PostStats_Id
        }
      });
  ...
  async deletePostStats(PostStats_Id: number){
	...
	async postUpvoteTracker(post: Post, user_Id: number) {
		let filteredList, found, postStats, upvotePluser, data = {};
		try {
      ...
      console.log("START POSTUPVOTEEEEEEEEEEEEEEE" + post.Post_Id)
			filteredList = await this.postUpvoteTrackerRepository.findAll({where: {Post_Id: post.Post_Id}});
      postStats = await this.postStatsRepository.findOne({ where: {PostStats_Id: post.PostStats_Id }});
			...
			if(filteredList == false){
				console.log("-------------------------------THE USER HAS NOT UPVOTED ON THIS YET");
				data = await this.postUpvoteTrackerRepository.create({Post_Id: post.Post_Id, User_Id: user_Id});

        upvotePluser = postStats.getDataValue("PostStats_Upvotes");
        upvotePluser++;
        await this.postStatsRepository.update({PostStats_Upvotes: upvotePluser}, {
          where: {
            PostStats_Id: post.PostStats_Id
				...
				found = await this.postUpvoteTrackerRepository.findAll({where: {Post_Id: post.Post_Id, User_Id: user_Id}});
				console.log("-----------------------------Found" + found);
			}
			if(found == false){
				console.log("-------------------------------THE USER HAS NOT UPVOTED ON THIS YET");
				data = await this.postUpvoteTrackerRepository.create({User_Id: user_Id, Post_Id: post.Post_Id});
        upvotePluser = postStats.getDataValue("PostStats_Upvotes");
        upvotePluser++;
        await this.postStatsRepository.update({PostStats_Upvotes: upvotePluser}, {
          where: {
            PostStats_Id: post.PostStats_Id
				...
				console.log("---------------------------------THE USER HAS UPVOTED ON THIS");
			}
  ...
  async getPostInformationByUserId(User_Id: number) {
    try {
      const post = await this.postRepository.findAll({where: {User_Id: User_Id}});
