...
export class ArticleRepository {
  ...
  private express: express.Application;
    ...
    this.express = express();
		...
		this.articleInformationRepository = this.db.sequelize.getRepository(ArticleInformation);
		...
		this.articleUpvoteTrackerRepository = this.db.sequelize.getRepository(ArticleUpvoteTracker);
	...
	async getArticles() {
		...
		async getFile(id: number) {
			try {
				const aInfo = await this.articleInformationRepository.findOne({where: {ArticleInformation_Id: id}});
	...
	async createArticle(article: Article, articleInformation: ArticleInformation, articleStats: ArticleStats, fileUrl: string) {
		...
		try {
			...
			aInfo = await this.articleInformationRepository.create(articleInformation);
			article.ArticleInformation_Id = aInfo.getDataValue('ArticleInformation_Id');
	...
	async updateArticle(article: Article, articleInformation: ArticleInformation, articleStats: ArticleStats) {
		...
		try {
			...
			await this.articleInformationRepository.update({...articleInformation}, {
				where: {
					ArticleInformation_Id: articleInformation.ArticleInformation_Id
				}
			});
			await this.articleStatsRepository.update({...articleStats}, {
				where: {
					ArticleStats_Id: articleStats.ArticleStats_Id
				}
			});
			data = await this.articleRepository.update({...article}, {
				where: {
	...
	async deleteArticle(Article_Id: number) {
		let aRef, aInfo, aStats, data = {};
		try {
			aRef = await this.articleRepository.findOne({where: {Article_Id: Article_Id}});
			aInfo = await this.articleInformationRepository.findOne({where: {ArticleInformation_Id: aRef.getDataValue("ArticleInformation_Id")}});
			aStats = await this.articleStatsRepository.findOne({where: {ArticleStats_Id: aRef.getDataValue("ArticleStats_Id")}});
	...
	async getArticleInformations() {
		try {
			const articleinformations = await this.articleInformationRepository.findAll();
	...
	async getArticleInformationById(ArticleInformationId: number) {
		try {
			const articleInfo = await this.articleInformationRepository.findOne({where: {ArticleInformation_Id: ArticleInformationId}});
			return articleInfo;
		} catch (error) {
			console.error(error);
			return [];
	...
	async createArticleInformation(articleinformation: ArticleInformation) {
		...
		try {
			data = await this.articleInformationRepository.create({
				...
				ArticleInformation_Description: articleinformation.ArticleInformation_Description,
				ArticleInformation_PublishedBy: articleinformation.ArticleInformation_PublishedBy
			});
		} catch (error) {
			console.error(error);
		}
	...
	async updateArticleInformation(articleinformation: ArticleInformation) {
		let data = {};
		try {
			data = await this.articleInformationRepository.update({...articleinformation}, {
				where: {
					ArticleInformation_Id: articleinformation.ArticleInformation_Id
				}
			});
	...
	async getArticleStatsById(ArticleStatsId: number) {
		try {
			const articleStats = await this.articleStatsRepository.findOne({where: {ArticleStats_Id: ArticleStatsId}});
			return articleStats;
		} catch (error) {
			console.error(error);
			return [];
	...
	async updateArticleStats(articlestats: ArticleStats) {
		...
		try {
			...
			data = await this.articleStatsRepository.update({...articlestats}, {
				where: {
					ArticleStats_Id: articlestats.ArticleStats_Id
				}
			});
	...
	async getArticleTopic() {
		...
		async getArticleTopicById(ArticleTopicId: number) {
			try {
				const articleTopic = await this.articleTopicRepository.findOne({where: {ArticleTopic_Id: ArticleTopicId}});
				return articleTopic;
			} catch (error) {
				console.error(error);
				return [];
	...
	async updateArticleTopic(articletopic: ArticleTopic) {
		let data = {};
		try {
			data = await this.articleTopicRepository.update({...articletopic}, {
				where: {
