...
const PostList = () => {
  ...
  useEffect(() => {
    axiosPrivate
      .get(`http://localhost:3081/api/postinformation`)
      .then((response) => {
  ...
  return (
    <>
      ...
      <table className="table table-striped table-hover">
        ...
        <tbody>
          {listPostInformation
            .filter((post: { PostInformation_Title: string }) => {
              //quick fix
              if (search === "") {
                return post;
              } else if (
                post.PostInformation_Title.toLowerCase().includes(
                  search.toLowerCase()
                )
              ) {
                return post;
            ...
            .map(
              (
                post: { PostInformation_Id: Key | null | undefined } //quick fix
              ) => (
