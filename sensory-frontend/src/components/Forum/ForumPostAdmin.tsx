...
const Post = ({ post }: { post: any }) => {
  ...
  useEffect(() => {
    ...
    handleClose();
    axiosPrivate
      .get(`http://localhost:3081/api/poststats/${post.PostInformation_Id}`)
      .then((response) => {
        setPostUpvotes(response.data.PostStats_Upvotes);
        setPostClicks(response.data.PostStats_Clicks);
      });
    axiosPrivate
      .get(`http://localhost:3081/api/post/${post.PostInformation_Id}`)
      .then((response) => {
        setPostUserId(response.data.User_Id);
        setPostId(response.data.Post_Id);
      });
    axiosPrivate
      .get(`http://localhost:3081/api/userinformation/${posterId}`)
      .then((response) => {
  ...
  const handleUpvote = async (e: any) => {
    ...
    const postRes = await axiosPrivate.get(
      `http://localhost:3081/api/post/${post.PostInformation_Id}`
    );
    const upvoteObject = {
      post: {
    ...
    await axiosPrivate.post(
      "http://localhost:3081/api/postUpvoteTracker",
      upvoteObject
    );
  ...
  const handlePostDetail = async (e: any) => {
    e.preventDefault();
    const getPostId = await axiosPrivate.get(
      `http://localhost:3081/api/post/${post.PostInformation_Id}`
    );
